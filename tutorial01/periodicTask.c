 ...
 * DETI/UA - RTOS - Tutorial 1: RT Services on Linux
 *
...
* ***********************************************/
uint64_t 
min_iat, max_iat; // Hold the minium/maximum observed inter arrival time
...
void * Thread_1_code(void *arg){
	...
	while(1) {
		...
		else if(niter > BOOT_ITER){ 	// Update max/min, if boot time elapsed 	    
...
int main(int argc, char *argv[]){
	...
	/* Process input args */
	if(argc != 3){
	  printf("Usage: %s PROCNAME PRIORITY, where PROCNAME is a string and PRIORITY a int\n\r", argv[0]);
	  return -1; 
	}

	/* A1 and A2 - Set task priority from argument #2*/
	prio = atoi(argv[2]);
...
void Heavy_Work(void){
	...
	struct timespec ts, tf; 		// Function start time, Function finish time
